...
@section('content')
							...
							<form id="departure_table_form" class="form-row" style="margin:0;">
								...
								<div class="col-md-3">
									<select id="departure_table_status" class="custom-select custom-select-sm form-control">
										...
										<option value="need_cancel_confirmation">Ожидает подтверждение отмены</option>
									</select>
								</div>
                                    ...
                                    @foreach($orders_vl as $order)
                                        <tr>
											...
											<td>
												...
												@if($order->transfer == 'individual')
													<span data-toggle="tooltip" title="Ценовая категория трансфера"><i class="fa fa-car" aria-hidden="true"></i></span>
												@endif
											</td>
											...
											@if($order->need_cancel_confirmation)
												<td>{!! trans('ordersmanagement.orders-table.status_order_need_cancel_confirmation') !!}</td>
												...
												<div class="btn-group pull-right btn-group-xs">
													...
													<div class="dropdown-menu dropdown-menu-right bg-white py-2">
														@role('admin')
															...
															<div class="dropdown-divider"></div>
															@if($order->need_cancel_confirmation)
																{!! Form::open(array('url' => 'orders/' . $order->id . '/cancel/confirm', 'class' => '', 'data-toggle' => 'tooltip', 'title' => 'Подтвердить отмену' )) !!}
																{!! Form::hidden('_method', 'GET') !!}
																{!! Form::hidden('cancel_info', $order->cancel_info) !!}
																{!! Form::button('Подтвердить отмену', array('class' => 'dropdown-item text-success','type' => 'button', 'style' =>'width: 100%;' ,'data-toggle' => 'modal', 'data-target' => '#confirmСancelConfirm', 'data-title' => 'Подтвердить отмену', 'data-message' => 'Вы уверены, что хотите изменить статус брони?')) !!}
																{!! Form::close() !!}
															@endif
                                ...
                                <tbody id="orders_table">
                                    @foreach($orders_pr as $order)
                                        <tr>
                                            <td>{{$order->id}}</td>
                                            <td>
												<span data-toggle="tooltip" title="{{$order->phone}}">
											...
											<td>
												@if($order->option_nameplate == 1)
													<span data-toggle="tooltip" title="Встреча с табличкой">
														<i class="fa fa-window-maximize" aria-hidden="true"></i>
													</span>
												@endif
											
												@if($order->option_babyk == 1)
													<span data-toggle="tooltip" title="Детское автокресло">
														<i class="fa fa-smile-o" aria-hidden="true"></i>
													</span>
												@endif
												
												@if($order->option_babyl == 1)
													<span data-toggle="tooltip" title="Детская автолюлька">
														<i class="fa fa-smile-o" aria-hidden="true"></i>
													</span>
												@endif
											...
											@elseif($order->ticket_child > 0)
												<td class="text-center">
													{{$order->tickets}}+{{$order->ticket_child}}
												</td>
											@else
												<td class="text-center">-</td>
											@endif
											<td>{{$order->airport_ukraine}}, {{$order->terminal}}</td>
											<td>
												{{ \Carbon\Carbon::parse($order->datetime)->format('d.m')}},
												{{ \Carbon\Carbon::parse($order->datetime)->format('H:i')}}
											</td>
											<td>
												<span data-toggle="tooltip" title="Вылетел с {{$order->airport_world}}">
													{{$order->flight}}
												</span>
											</td>
	...
	@include('modals.modal-cancel-confirm')
...
@section('footer_scripts')
    ...
    @include('scripts.cancel-confirm-modal-script')
