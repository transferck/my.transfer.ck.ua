...
@if($order->airport_ukraine == 'Б')
	<p><b>{!! trans('forms.create_order_label_airportvl') !!}: </b>Борисполь, {{$order->terminal}}</p>
@elseif($order->airport_ukraine == 'Ж')
	<p><b>{!! trans('forms.create_order_label_airportvl') !!}: </b>Жуляны</p>
@else($order->airport_ukraine == 0)
	<p><b>{!! trans('forms.create_order_label_airportvl') !!}: </b>-</p>
@endif

<p><b>{!! trans('forms.create_order_label_datetimevl') !!}: </b>			{{ \Carbon\Carbon::parse($order->datetime)->format('d.m.Y, H:i')}}</p>
@if($order->type == 'vl')
