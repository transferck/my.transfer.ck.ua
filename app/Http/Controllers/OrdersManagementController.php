...
class OrdersManagementController extends Controller
{
    ...
    public function ordersApi(Request $request)
    {
        ...
        if (isset($filters['status'])) {
            if ($filters['status'] == 'need_cancel_confirmation') {
                $ordersQuery->where('need_cancel_confirmation', true);
            } else {
                $ordersQuery->where('status_order', $filters['status']);
            }
        }
    ...
    public function allOrdersApi(Request $request)
    {
        ...
        if (is_array($filters)) {
            ...
            if ( isset($filters['status']) ) {
                if ($filters['status'] == 'need_cancel_confirmation') {
                    $orders->where('need_cancel_confirmation', true);
                } else {
    ...
    public function create(Request $request)
    {
        ...
        $transferAutoTypes = array_merge(Order::TRANSFER_AUTO_PRICES['Б'], Order::TRANSFER_AUTO_PRICES['Ж']);
        $activeTab = $request->cookie('tab', 'vl');

        return view('ordersmanagement.create-order')->with(
            compact('roles', 'showCardGold', 'showDiscountHalf', 'transferAutoTypes', 'activeTab')
        );
    }
    ...
    public function store(Request $request)
    {
        ...
        $validatorArr = [
            ...
            'tickets'                			=> 'required',
            'transfer'                			=> 'required',
            'transfer_type_auto'                => 'required_if:transfer,individual',
        ];
        ...
        $validator = Validator::make($request->all(),
            ...
            [
				...
				'time.required'        		  	    => trans('ordersmanagement.messages.datetime'),
				'date.required'        		  	    => trans('ordersmanagement.messages.datetime'),
        ...
        if ($validator->fails()) {
            return back()->withErrors($validator)
                ->withInput()
                ->withCookie(new Cookie('tab', 'vl', (new \DateTime())->modify('+20 seconds')));
        }
        // Create Order
		$order = new Order;
		...
		$order->type = 'vl';
        $order->datetime = $request->date . ' ' . $request->time;
        ...
        if ($order->transfer_type_auto = $request->transfer_type_auto) {
            $totalHumans = intval($request->tickets) + intval($request->ticket_child);

            $autosCount = $totalHumans /
                Order::TRANSFER_AUTO_HUMANS[$order->airport_ukraine][$order->transfer_type_auto];

            $autosCount = intval(ceil($autosCount));

            $order->transfer_price = $autosCount *
                Order::TRANSFER_AUTO_PRICES[$order->airport_ukraine][$order->transfer_type_auto];
        }
    ...
    public function store_pr(Request $request)
    {
        ...
        if ($validator->fails()) {
            return back()->withErrors($validator)
                ->withInput()
                ->withCookie(new Cookie('tab', 'pr', (new \DateTime())->modify('+20 seconds')));
        }
        // Create Order
		$order = new Order;
		$order->fio = $request->fio;
		$order->phone = $request->phone;
        $order->fio2 = $request->fio2;
        $order->phone2 = $request->phone2;
		$order->type = 'pr';
        $order->datetime = $request->date . ' ' . $request->time;
		$order->airport_ukraine = $request->airport_ukraine;
		$order->airport_world = $request->airport_world;
		$order->terminal = $request->terminal;
		$order->registration = $request->registration;
		$order->flight = $request->flight;
		$order->address = $request->address;
		$order->address2 = $request->address;
		$order->tickets = $request->tickets;
		$order->ticket_full = $request->ticket_full;
		$order->ticket_child = $request->ticket_child;
		$order->ticketfree_reason = $request->ticketfree_reason;
		$order->children = $request->children;
		$order->children_place = $request->children_place;
		$order->transfer = $request->transfer;
		$order->suburb = $request->suburb;
		$order->info = $request->info;
		$order->status_order = 'sent';
		$order->status_pay = $request->status_pay;
		$order->option_nameplate = $request->option_nameplate;
		$order->option_babyk = $request->option_babyk;
		$order->option_babyl = $request->option_babyl;
		$order->user_id = auth()->user()->id;
    ...
    public function cancelOrder(Request $request, $id)
    {
        ...
        if (Auth::user()->isRole(['operator', 'admin'])) {
            $order->status_order = 'cancel';
            $order->need_cancel_confirmation = false;
        } else {
            $order->need_cancel_confirmation = true;
        }
     ...
     * @throws AuthorizationException
     */
    public function confirmCancelOrder(int $id)
    {
        if (!Auth::user()->isRole(['operator', 'admin'])) {
            throw new AuthorizationException('Forbidden');
        }
        ...
        return back()->with('success', trans('ordersmanagement.cancelSuccess'));
    }
